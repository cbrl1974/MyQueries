...
SELECT top 150
  ...
  --    , SUBSTRING(
  --  text, 
  --  CHARINDEX('MerchantID: ', text) + LEN('MerchantID: '), 
  --  CHARINDEX('!', text) - (CHARINDEX('MerchantID: ', text) + LEN('MerchantID: '))
  --) AS MerchantID
      , [Time]
      , [Severity]
      , [Module]
...
where 1= 1
	...
	--and time between '2025-05-22 00:00:00.000' and '2025-05-22 23:59:59.999'
	and  time > convert(date,getdate()-1)
order by [time] desc
...
select manufacturerIdentifier from products where id_product = 577574
...
select ManufacturerIdentifier from products where id_product =805314
...
union all
	...
	select * from products where ManufacturerIdentifier = 'D501-02X2'
