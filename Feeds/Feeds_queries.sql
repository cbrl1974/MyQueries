...
select * from devtailbasecore.feeds.merchantFeeds where  feedid = 9 order by merchantid desc
select * from devtailbasecore.feeds.merchantFeeds where merchantid =1193 and feedid = 9
...
where FeedId = 9  
and merchantid = 3336
and productid = 552829
order by ProductBasePriceId desc

--****************** Today 16494
--delete top(16212) from devtailbasecore.feeds.ProductBasePrices where FeedId = 9  
--and productid = 465275 order by ProductBasePriceId desc

select * from devtailbasecore.feeds.ProductBasePrices where FeedId = 9   and merchantid is null
