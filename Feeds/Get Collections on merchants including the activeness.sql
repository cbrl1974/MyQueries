--select * from feeds.merchantfeeds where merchantid = 589 and feedid  = 1

declare @MerchantId int=1448;
declare @FeedBrandCategories as FeedBrandCategory;
insert into @FeedBrandCategories values (2768,'58,318,419,134,175,208,258,315,311,312,313,314,316,319,589,649,178,210,209,179,429,431,519,181,182,183,184,186,187,277,587,607,188,189,190,191,193,407,591,260,294,298,282,590');
declare @FeedBrandDepartments as FeedBrandCategory;
insert into @FeedBrandDepartments values (2768,'28,26');
declare @FeedId int=1;
declare @LocationName nvarchar(1000)='CMC';
...
AS (
    ...
    and CollectionContainsOnlyActiveProducts = 1
    order by CollectionIsActiveAndComplete