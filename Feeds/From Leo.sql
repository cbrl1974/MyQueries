-- check all Coaster collections
select distinct mc.collectionID
FROM dbo.merchantCollections mc
JOIN dbo.collections c ON mc.collectionID = c.ID
join collection_product cp on cp.collectionid = c.id
join products p on p.id_product = cp.productid
WHERE 
    mc.Merchant_ID = 1448
    AND (
        p.manufid in (3181,3182,3184,4226,4227,1436)
    );
    
    
-- check all complete and incomplete collections
SELECT 
    distinct mc.ID AS MerchantCollectionID,
    mc.collectionID,
    COUNT(DISTINCT cp.productID) AS total_required_products,
    COUNT(DISTINCT mp.productID) AS merchant_products_in_collection
FROM merchantCollections mc
JOIN collection_product cp
    ON mc.collectionID = cp.collectionID
JOIN collections c
    ON mc.collectionID = c.ID
join products
	p on p.id_product = cp.productid
LEFT JOIN dbo.MerchantProds mp
    ON mp.merchant_id = mc.Merchant_ID
    AND mp.productID = cp.productID
WHERE 
    mc.Merchant_ID = 1448
    AND (
        p.manufid in (3181,3182,3184,4226,4227,1436)
    )
GROUP BY mc.ID, mc.collectionID
--HAVING COUNT(DISTINCT cp.productID) > COUNT(DISTINCT mp.productID); ---> incompletes
HAVING COUNT(DISTINCT cp.productID) = COUNT(DISTINCT mp.productID); ---> completes

select * from collection_product where collectionid = 3176
select * from merchantprods where merchant_id = 589 and productid in (select productid from collection_product where collectionid = 3176)
...
AS (
	...
	--select * from products where id_product in (655199,698113,788370)
