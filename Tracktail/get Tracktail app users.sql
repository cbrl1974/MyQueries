...
SELECT * FROM [TrackTailApp].[AspNetUsers] where Email like '%cityfurniture%'

SELECT * FROM [TrackTailApp].ClientAccounts where merchantid = 1202

SELECT * FROM [TrackTailApp].ClientAccounts  where AccountId in (525,224)

update top (4) [TrackTailApp].[AspNetUsers]
set ClientId = 224
where Email like '%cityfurniture%'

SELECT * FROM [TrackTailApp].[AspNetUsers] where Email like '%tailbase%';
SELECT * FROM [TrackTailApp].ClientAccounts where merchantid = 1202
SELECT * FROM [TrackTailApp].ClientAccounts where merchantid = 589
SELECT top 100  * FROM TrackTailDb.Tracktail_Field where merchantid = 1202
SELECT top 100  * FROM TrackTailDb.Tracktail_Field where merchantid = 589

SELECT top 100 *  from  TrackTailSolutions_Clients where merchantid = 1448
...
SELECT u.UserName, r.Name FROM [TrackTailApp].[aspnetuserroles] ur
...
SELECT TCA.[ID]
                            ...
                            , (
                                ...
                                (select top 1 brand from merchantFeeds where merchant_id = TCA.[MerchantId] and brand = 'NECO_NE' and active = 1) AS 'NECO_NE_brand',
                                (select top 1 brand from MerchantFeedWarehouses where merchant_id = TCA.[MerchantId] and active = 1 and brand = 'ADC') AS 'NECO_ADC_brand'

                        FROM TrackTailSolutions_Clients TCA
                        JOIN merchants DDM ON DDM.ID = TCA.MerchantId
                        JOIN Companies DDC ON DDC.Id_cie = TCA.CieId
                        WHERE TCA.[CountryCode] is null
                        --WHERE TCA.ID = 483